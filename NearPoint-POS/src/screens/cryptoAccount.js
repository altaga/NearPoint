...
// Assets
import eth from "../assets/eth-token-icon.png"
...
import { contentColor, native, NODE_ENV_NETWORK_NAME, NODE_ENV_NETWORK_RCP, NODE_ENV_CHAINLINK_FEED_CONTRACT, NODE_ENV_DATA_FEEDS_RCP, tokens, tokensContracts, tokensIcons, headerColor } from "../../env"
...
async function getAURORA() {
    return new Promise((resolve, reject) => {
        ...
        fetch("https://api.coingecko.com/api/v3/simple/price?ids=aurora-near&vs_currencies=usd", requestOptions)
            .then(response => response.text())
            .then(result => resolve(JSON.parse(result)["aurora-near"].usd))
...
async function getNEAR() {
    return new Promise((resolve, reject) => {
        ...
        fetch("https://api.coingecko.com/api/v3/simple/price?ids=wrapped-near&vs_currencies=usd", requestOptions)
            .then(response => response.text())
            .then(result => resolve(JSON.parse(result)["wrapped-near"].usd))
            .catch(error => console.log('error', error));
    })
}
...
class CryptoAccount extends Component {
    ...
    async componentDidMount() {
        this.props.navigation.addListener('focus', async () => {
            ...
            let priceETH = await this.contract.methods.getLatestETHPrice().call()
            this.context.setValue({
                            ...
                            <ScrollView persistentScrollbar>
                                <View style={{ flexDirection: "row", justifyContent: "center", alignItems: "center" }}>
                                    <View style={{ width: "33.33%" }}>
                                        {
                                            this.context.value.show ? <Image source={eth} style={{ width: 20, height: 20, alignSelf: 'center' }} /> : <Text style={{ fontSize: 20, color: 'white', alignSelf: 'center' }}>{"***"} </Text>
                                        }
                                    </View>
