...
import { geckoNative, geckoTokens, NODE_ENV_RCP, NODE_ENV_CHAINLINK_FEED_CONTRACT, NODE_ENV_DATA_FEEDS_RCP, tokens, tokensContracts, contentColor } from "../../env"
...
import aurora from "../assets/aurora-token-icon.png";
import near from "../assets/near-token-icon.png"
...
class Landing extends Component {
    constructor(props) {
        ...
        this.state = {
            // Price Feeds
            prices: [],
            symbol: ["AURORA", "AVAX", "BNB", "BTC", "DOT", "ETH", "LINK", "MATIC", "NEAR", "NEO", "SOL", "USDC", "XRP"],
            icons: [aurora, avax, bnb, btc, dot, eth, link, matic, near, neo, sol, usdc, xrp],
        };
        reactAutobind(this)
        this.axios = require('axios');
        this.source = this.axios.CancelToken.source();
        this.web3 = new Web3(NODE_ENV_RCP)
    ...
    async updatePriceFeeds() {
        ...
        // To be improve
        let results = await getUSD(geckoTokens)
        let tokenUSD = {}
        geckoTokens.map((item, index) => tokenUSD[tokens[index]] = results[item].usd)
        const aurora = tokenUSD["AURORA"]
        const near = tokenUSD["NEAR"]
        this.mount && this.setState({
            prices: [
                ...
                (parseFloat((priceUSDC).toString()) / 10000000),
                ...
                <View style={GlobalStyles.container}>
                    ...
                    <View style={GlobalStyles.mainSub}>
                        <View style={{ height: "20%", marginTop: 30 }}>
                            ...
                            <View style={{ backgroundColor: contentColor, height: 1, width: "90%", alignSelf: "center", marginTop: 30 }} />
                        </View>
                                            ...
                                            this.state.prices.map((price, index) => {
                                                return (
                                                    <View key={"Feed:" + index} style={[{ width: "100%" }]}>
                                                        <View style={{ marginVertical: 10, marginHorizontal: 20, flexDirection: "row", justifyContent: "space-evenly", alignItems: "center", backgroundColor: contentColor, height: 50, borderRadius: 100 }}>
